...
import utils
from dil_transformer_net import TransformerNet
...
def train(start_epoch = 0):
    ...
    all_style_img_paths = [os.path.join(enums.style_image_dir, f) for f in sorted(os.listdir(enums.style_image_dir))]
    all_style_grams = {}
    ...
    for e in range(start_epoch, enums.epochs):
        ...
        for batch_id, (x, _) in enumerate(train_loader):
            ...
            if (batch_id + 1) % enums.log_interval == 0:
                ...
                print(mesg)
                con_loss.append(agg_content_loss / (overall_cnt + 1))
                content_losses = np.array(con_loss)
                sty_loss.append(agg_style_loss / (overall_cnt + 1))
                style_losses = np.array(sty_loss)
                tot_loss.append((agg_content_loss + agg_style_loss) / (overall_cnt + 1))
                total_losses = np.array(tot_loss)
                plot_cnt += 1
                plt.figure(1)
                plt.clf()
                plt.plot(np.arange(plot_cnt)+1, content_losses, color='r', label='Content')
                plt.plot(np.arange(plot_cnt)+1, style_losses, color='b', label='Style')
                plt.plot(np.arange(plot_cnt)+1, total_losses, color='g', label='Total')
                plt.xlabel('Iterations')
                plt.ylabel('Loss')
                plt.legend()
                plt.title('Conditional Instance Normalization')
                if plot_cnt <= 999:
                    plt.savefig(enums.PLOT_PATH)
                else:
                    plt.savefig(enums.PLOT_2EP)
                overall_cnt += 1
            time_elapsed_batch = time.time() - since_batch
            print "Time elapsed for 1 batch: ", time_elapsed_batch
